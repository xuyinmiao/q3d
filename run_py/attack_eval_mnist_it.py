...
data_dir = os.path.join(project_root, "data_mnist")
os.makedirs(data_dir, exist_ok=True)

save_dir = os.path.join(project_root, "model_history_mnist")
...
hybrid_model = HybridQCNN(n_qubits=8, n_layers=3, n_classes=10).to(device)
...
# ===================== 设置迭代次数和固定扰动 =====================
iterations_range = np.arange(0, 21, 4)  # 0, 5, 10, 15, ..., 50
fixed_epsilon = 0.15  # 固定扰动规模
...
for attack_key, attack_config in attack_configs.items():
    ...
    for num_iter in iterations_range:
        ...
        if num_iter == 0:
            ...
            console.print(f"[bold cyan]→ Generating adversarial set for Hybrid QCNN[/bold cyan]")
            hybrid_adv_loader = generate_adversarial_loader(
                hybrid_model, val_loader, fixed_epsilon,
                attack_type=attack_key, model_name="Hybrid QCNN", **attack_params
            )

            console.print(f"[bold green]→ Generating adversarial set for Classical CNN[/bold green]")
            classical_adv_loader = generate_adversarial_loader(
                classical_model, val_loader, fixed_epsilon,
                attack_type=attack_key, model_name="Classical CNN", **attack_params
            )

        console.print(f"\n[bold cyan]→ Evaluating Hybrid QCNN[/bold cyan]")
        hybrid_acc = evaluate_and_collect(hybrid_model, hybrid_adv_loader, f"{attack_name} iter={num_iter}")
        
        console.print(f"\n[bold green]→ Evaluating Classical CNN[/bold green]")
...
plt.title(f"Adversarial Robustness vs. Iterations (MNIST, ε={fixed_epsilon})", fontsize=18, fontweight='bold', pad=20)
...
# 选择关键迭代点：每隔一个点采样
table_iterations = iterations_range
...
for idx, num_iter in enumerate(table_iterations):
    ...
    for attack_key, results in all_results.items():
        attack_name = results['name']
        # 在 iterations_range 中查找当前 num_iter 的真实索引
        idx_in_range = int(np.where(iterations_range == num_iter)[0][0])
        hybrid_acc = results['hybrid_accs'][idx_in_range]
        classical_acc = results['classical_accs'][idx_in_range]
...
with open(results_file, 'w') as f:
    ...
    for attack_key, results in all_results.items():
        ...
        f.write("-" * 80 + "\n")
        f.write(f"{'Iterations':<12} {'Hybrid QCNN':<15} {'CNN':<15}\n")
        f.write("-" * 80 + "\n")
