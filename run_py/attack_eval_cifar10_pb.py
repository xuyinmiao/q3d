...
hybrid_model = HybridQWideResNet(n_classes=10, n_layers=6, n_qubits=8).to(device)
classical_model = WideResNet(n_classes=10, latent_dim=8).to(device)
...
for attack_key, attack_config in attack_configs.items():
    ...
    for eps in epsilons:
        ...
        # ====== 为 Hybrid QWideResNet 生成针对性的对抗样本 ======
        console.print(f"\n[bold cyan]→ Generating adversarial set for Hybrid QWideResNet[/bold cyan]")
        hybrid_adv_loader = generate_adversarial_loader(
            hybrid_model, val_loader, eps, 
            attack_type=attack_key, 
            model_name="Hybrid QWideResNet",
            **attack_params
        )
        
        console.print(f"[bold cyan]→ Evaluating Hybrid QWideResNet on adversarial examples[/bold cyan]")
        hybrid_acc = evaluate_and_collect(hybrid_model, hybrid_adv_loader, f"{attack_name} ε={eps:.3f}")
        
        # ====== 为 WideResNet 生成针对性的对抗样本 ======
        console.print(f"\n[bold green]→ Generating adversarial set for WideResNet[/bold green]")
        ...
        console.print(f"[bold green]→ Evaluating WideResNet on adversarial examples[/bold green]")
...
plt.title("Adversarial Robustness Comparison (CIFAR10)", fontsize=18, fontweight='bold', pad=20)
...
with open(results_file, 'w') as f:
    ...
    for attack_key, results in all_results.items():
        ...
        f.write("-" * 80 + "\n")
        f.write(f"{'Epsilon':<12} {'Hybrid QWideResNet':<15} {'WideResNet':<15}\n")
        f.write("-" * 80 + "\n")
