...
# ===================== 设置迭代次数和固定扰动 =====================
iterations_range = np.arange(0, 11, 1)  # 0, 5, 10, 15, ..., 50
...
plt.title(f"Adversarial Robustness vs. Iterations (Fashion-MNIST, ε={fixed_epsilon})", fontsize=18, fontweight='bold', pad=20)
...
# 选择关键迭代点：每隔一个点采样
table_iterations = iterations_range[::2]  # 自动从iterations_range中采样
...
for idx, num_iter in enumerate(table_iterations):
    ...
    for attack_key, results in all_results.items():
        attack_name = results['name']
        # 找到对应的迭代次数在原iterations_range中的索引
        original_idx = idx * 2  # 因为我们是每隔一个采样
        hybrid_acc = results['hybrid_accs'][original_idx]
        classical_acc = results['classical_accs'][original_idx]
