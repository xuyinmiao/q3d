...
from network import HybridQCNN
...
fmnist_dataset = FashionMNIST(root=data_dir, train=True, download=True, transform=transform)

val_size = int(0.2 * len(fmnist_dataset))
train_size = len(fmnist_dataset) - val_size

generator = torch.Generator().manual_seed(42)
train_dataset, val_dataset = random_split(fmnist_dataset, [train_size, val_size], generator = generator)
...
# ===================== 训练准备 =====================
model = HybridQCNN(n_qubits=8, n_layers=5, n_classes=10)
...
console = Console()
console.rule("[bold cyan]🚀 Hybrid QCNN Training Started[/bold cyan]")
console.print(Panel.fit(
    "[bold bright_cyan]Model:[/bold bright_cyan] Hybrid QCNN\n"
...
for epoch in range(num_epochs):
    ...
    if val_accuracy > best_val_acc:
        ...
        best_epoch = epoch
        model_path = os.path.join(save_dir, "best_hybrid_qcnn_model.pth")
...
# ===================== 保存训练过程 =====================
history_path = os.path.join(save_dir, "hybrid_qcnn_history.json")
...
plt.tight_layout()
plot_path = os.path.join(save_dir, "qcnn_training_history.png")
