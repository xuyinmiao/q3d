...
from network import HybridQWideResNet
...
cifar100_dataset = CIFAR100(root=data_dir, train=True, download=True)

val_size = int(0.2 * len(cifar100_dataset))
train_size = len(cifar100_dataset) - val_size

generator = torch.Generator().manual_seed(42)
train_subset, val_subset = random_split(range(len(cifar100_dataset)), [train_size, val_size], generator=generator)
...
train_dataset = TransformedSubset(cifar100_dataset, train_indices, transform_train)
val_dataset = TransformedSubset(cifar100_dataset, val_indices, transform_val)
...
# ===================== 训练准备 =====================
model = HybridQWideResNet(n_classes=100, n_layers=6, n_qubits=8)  
...
console = Console()
console.rule("[bold cyan]🚀 Hybrid Q-WideResNet Training Started[/bold cyan]")
console.print(Panel.fit(
    "[bold bright_cyan]Model:[/bold bright_cyan] Hybrid Q-WideResNet\n"
...
for epoch in range(num_epochs):
    ...
    if val_accuracy > best_val_acc:
        ...
        best_epoch = epoch
        model_path = os.path.join(save_dir, "best_hybrid_qwideresnet.pth")
...
# ===================== 保存训练过程 =====================
history_path = os.path.join(save_dir, "hybrid_qwideresnet_history.json")
...
plt.tight_layout()
plot_path = os.path.join(save_dir, "hybrid_qwideresnet_history.png")
